...
const rounds = [
  { type: "image-grid", instruction: "Select all squares containing Traffic Lights" },
  { type: "image-grid", instruction: "Select all squares containing Bicycles" },
  { type: "checkbox",   instruction: "Please confirm the following to proceed" },
  { type: "image-grid", instruction: "Select all squares containing Crosswalks" },
]
...
const failMessages = [
  ...
  "Verification complete. Redirecting to checkout...",
]

const checkboxItems = [
  "I am not a robot",
//   "I accept my questionable food choices",
//   "I acknowledge I have been stress-eating lately",
  ...
  const handleVerify = () => {
    ...
    setTimeout(() => {
      ...
      if (round < rounds.length - 1) {
        ...
        setTimeout(() => router.push("/loading-screen"), 1500)
      }
    }, 1800)
  }
      ...
      >
        ...
        {error && (
          <p className={`text-sm text-center mb-3 font-medium
            ${error.includes("Redirecting") ? "text-green-500" : "text-red-500"}`}>
            {error}
          </p>
        )}
